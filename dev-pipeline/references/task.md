...
- **按状态转移更新**：进入新状态、计划版本变化、批准状态变化、切片状态变化、`ContextGather` 证据包、`PlanReview` 发现项、`UpdatePlan`、`Review` 发现项、`FixFindings`、验证失败、交付修复回流、规则沉淀检查、阻塞搁置或回流时更新，不等最后补总结。
...
- 高约束主干启用任务台账时，必须至少记录：`blocking_delegate_guard`、`prompt_template_guard`、`context_pack_guard`、`plan_quality_guard`、`plan_approval_guard`、`minor_note_guard`、`review_gate_guard`、`artifact_freshness_guard`、`verification_guard`、`explicit_command_guard`、`delivery_ready_guard`、`deliver_repair_guard`、`rule_distillation_guard`。
- 未命中某个守卫条件时写 `not_applicable` 和依据。
...
- trigger: <用户纠正 / review 发现项 / 测试失败 / 子代理发现 / none>
