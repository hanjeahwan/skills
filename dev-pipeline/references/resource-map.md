...
- `task-record.md`：只有需要启用 `.dev-pipeline/` 工作态记录时读取；只读模式、`OffRamp`、单文件低风险任务默认不需要。

## 主干阶段细节

- `statechart.md`：需要理解主状态机、状态转移、守卫条件、动作、退出条件，或处理回流、子代理 `join_point`、验证记录时读取。
- `implementation.md`：进入 Stage 3 写代码时读取，覆盖单一事实来源、状态机建模、阶段边界和命名规则。
