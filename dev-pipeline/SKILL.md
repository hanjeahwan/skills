...
- 按验证选择矩阵执行；没做的验证要明确说。
- `Verify` 由主线程执行实际验证，例如定向测试、typecheck、lint、build、手动路径或未验证项记录；`Review` 不能替代 `Verify`。
...
- `dev server`、`ng serve`、`pnpm start` 是长运行本地副作用，默认不启动。只有用户明确要可访问 URL，或手动/浏览器验证必须依赖它且登录态、测试数据、端口归属和停止策略都明确时才启动。
