...
6. Styling behavior tests for every applicable Ant `style/**` branch and delegated style branch: orientation, mode, placement, variant, status, RTL, token units, semantic-slot placement, baseline/axis alignment, connector endpoints, containing blocks, and responsive layout.
...
Use independent subagents for work that benefits from separation between implementation and verification. Record each branch in the contract's verification ownership table.

| Branch | Use when | Expected output |
|---|---|---|
| Source/API audit | Public props, exports, defaults, rc delegation, or semantic DOM are non-trivial | Source-backed API rows and missing-surface findings |
| Style graph audit | Layout, tokens, status, variant, connector, overlay, or responsive styling matters | Style matrix rows and source CSS behavior notes |
| Docs demo parity audit | Docs demos are part of acceptance | Source demo list, target fixture mapping, shell/grid/card parity findings |
| Browser geometry verifier | Visual position, connectors, portals, focus rings, underlines, handles, or overlays can drift | DOM/CSS measurements that try to falsify pass claims |
| Contract closure audit | The migration is broad, visual, or user-facing | Checklist rows that are pass, source-backed non-goal, or blocked with exact reasons |
| Learning regression audit | A user correction, verifier finding, or repeated issue generated a learning | Regression assertion mapped to test, contract row, checklist row, benchmark assertion, or source-backed non-goal |
...
- Final acceptance requires a fresh-context independent verifier subagent report. If subagents are unavailable, the migration remains `Migration blocked` until the user explicitly approves a fallback reviewer path in the contract.
...
- Any user-approved fallback reviewer pass must use the same table and record the fallback reason. The fallback still needs source-backed measurements, not just a screenshot.
...
| Subagent side-branch audits completed | <status> | <subagent notes or user-approved fallback reason> | <branch outputs merged into contract/tests or blocker> | <verifier subagent or source-backed reviewer> |
...
- Verifier: <fresh-context subagent id/model or user-approved fallback>
- Final verdict: <verified-pass | blocked>
